...
export function ChatMemorySection() {
  ...
  return (
    <Wrap title={`Память чата${items ? ` · ${items.length}` : ""}`}>
      <Note>Долговременные факты о тебе — имя, предпочтения, рабочие детали. Часть агент извлекает из диалога сам, часть добавляешь вручную; их использует поиск по фактам. Смысловая память (эпизоды) живёт отдельно, в RAG.</Note>
      ...
      {items === null ? (
        ...
        <Note>Пока пусто. Факты появляются по ходу диалога или добавь вручную.</Note>
      ) : (
