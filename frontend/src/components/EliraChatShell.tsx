          ...
          <div className="chat-header-row">
            ...
            {sideTab === "chats" && (
              <div className="chat-header-actions icon-actions" style={{display:"flex"}}>
                ...
                {/* Tasks dock — always available on the chat surface, lives in
                    this panel (not a separate floating button). */}
                {(() => { const activeTasks = tasksList.filter(t => t.status !== "done"); return (
                  <div style={{position:"relative"}}>
                    <button className={`soft-btn icon-btn ${taskDockOpen ? "on" : ""}`} title="Задачи" onClick={()=>{ const n = !taskDockOpen; setTaskDockOpen(n); if (n) loadTasks(); }} style={{position:"relative"}}>
                      <UiIcon icon={ListTodo} size={14} />
                      {activeTasks.length > 0 && <span style={{position:"absolute",top:-4,right:-4,minWidth:15,height:15,borderRadius:8,background:"var(--accent)",color:"#fff",fontSize:8,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 3px"}}>{activeTasks.length}</span>}
                    </button>
                    {taskDockOpen && (
                      <div style={{position:"absolute",top:"100%",right:0,zIndex:99,marginTop:4,width:286,maxHeight:360,overflow:"auto",background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:12,boxShadow:"0 8px 28px rgba(0,0,0,.3)",padding:10}}>
