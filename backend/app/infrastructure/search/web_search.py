...
# ---------------------------------------------------------------------------
# Legacy web search (single-pass, DDG fallback)
# ---------------------------------------------------------------------------
...
def do_web_search_legacy(
    ...
    *,
    ...
    for item in search_results[:7]:
        url = item["url"]
        if url not in all_urls_seen and not any(d in url for d in WEB_SKIP_FETCH_DOMAINS):
    ...
    if targets:
        ...
        with ThreadPoolExecutor(max_workers=min(len(targets), 4)) as executor:
            future_map = {executor.submit(core_fetch_fn, t["url"]): t for t in targets}
    ...
    _tl(
        timeline, "tool_web", "Веб-поиск", "done",
        f"{len(search_results)} найдено ({engines_str}), "
        f"{fetched_count} страниц загружено, {len(news_results)} новостей",
    )
