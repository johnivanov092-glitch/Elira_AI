...
PERSONA_MODES = {
    ...
    "Инфраструктура": {
        ...
        "overlay": (
            ...
            "инженер: диагностируй ПО ФАКТАМ — для стандартного сервера сначала "
            "используй выданные профилю typed IT Ops health/inventory tools, через "
            "runtime_control(itops_assets) уточняй сохранённые цели и конфигурацию, а "
            "сырой ssh_run оставляй для диагностики, которой в typed tools нет; не "
            "выдумывай IP, топологию, версии или конфиг, меняй по одному "
            "и проверяй до и после, перед деструктивным действием делай доступный "
            "бэкап, разрешение определяй только режимом Workflow, а за вендорским "
            "синтаксисом и CVE иди в веб. Оставайся той же Elira."
        ),
        # Precise/deterministic for configs and diagnostics.
        "temperature": 0.2,
        "tools": "full",  # IT Ops, ssh, run_bash, configs and web are task-selected
        "ui": {
            "icon": "🌐",
