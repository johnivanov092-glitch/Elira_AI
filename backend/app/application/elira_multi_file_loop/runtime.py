"""Application-layer runtime for the Elira phase19 multi-file dev-loop route.

Owns the SQLite schema, JSON helpers, project scanner, plan/reasoning/file-op/
verify builders, and persistence used by ``/api/elira/phase19/*``. The HTTP
layer in ``api/routes/elira_phase19.py`` is a thin FastAPI shell that defines
the router + the Pydantic request schema and delegates everything else here.
"""
...
# ───────── Reasoning / Plan / File ops / Verify ─────────
...
def prepare_run(goal: str, mode: str, selected_paths: List[str]) -> dict:
    """Build the phase19 multi-file run payload, persist it, and return the response body.

    Mirrors the previous body of ``POST /api/elira/phase19/run``.
    """
    project_files = scan_project()
    reasoning = build_project_reasoning(goal, selected_paths)
    plan = build_multi_file_plan(goal, selected_paths, project_files)
    ...
    run_id = persist(goal, mode, selected_paths, plan, reasoning, file_ops, verify)
