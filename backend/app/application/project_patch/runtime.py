...
class ProjectPatchRuntime:
    def __init__(
        self,
        *,
        project_root: Path,
        read_project_file_func: Callable[..., dict[str, Any]],
        write_project_file_func: Callable[..., dict[str, Any]],
        time_func: Callable[[], float] | None = None,
    ) -> None:
        self.project_root = project_root
        self._read_project_file = read_project_file_func
        self._write_project_file = write_project_file_func
    ...
    def preview_patch(self, path: str, new_content: str, max_chars: int = 20000) -> dict[str, Any]:
        current = self._read_project_file(path, max_chars=max_chars)
    ...
    def apply_patch(self, path: str, new_content: str) -> dict[str, Any]:
        ...
        result = self._write_project_file(path, new_content)
        ...
        verify = self._read_project_file(path, max_chars=500000)
    ...
    def rollback_patch(self, path: str, backup_id: str) -> dict[str, Any]:
        ...
        result = self._write_project_file(path, original_content)
        return {
    ...
    def _create_backup(self, path: str, old_content: str) -> dict[str, Any]:
        backup_id = f"{int(self._time() * 1000)}_{self._safe_name(path)}"
        backup_meta = {
            "backup_id": backup_id,
            "path": path,
