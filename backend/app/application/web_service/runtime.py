# -*- coding: utf-8 -*-
"""Application-layer runtime — legacy search compatibility wrapper.

Keeps older tool/service callers working while the real source of truth
for search orchestration lives in ``core/web.py``.  No HTTP at module
level; ``core/web`` calls happen inside the functions.
"""
...
def search_web(query: str, max_results: int = 8) -> dict[str, Any]:
    """Search via core/web and return a normalised result dict.

    Returns:
      ok, query, sources, engines_used, context, count, engine_links
    """
...
def research_web(query: str, max_results: int = 8):
    """Deep research via core/web (downloads and summarises pages)."""
