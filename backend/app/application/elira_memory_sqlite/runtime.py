# -*- coding: utf-8 -*-
"""Application-layer runtime for the Elira memory SQLite service.

Owns DB_PATH, DEFAULT_CHAT_TITLE, _VALID_TABLES, connection wiring, and all
wrapper functions that delegate to ``application/elira_memory/runtime.py``.
Pure Python — no HTTP, no FastAPI imports.

``init_db()`` is called at module level to bootstrap the schema on import,
preserving the original side-effect behaviour of ``services/elira_memory_sqlite``.
"""
