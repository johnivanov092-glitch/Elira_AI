# -*- coding: utf-8 -*-
"""Application-layer runtime for Agent Monitor / Monitoring (Agent OS Phase 5).

Owns DB_PATH, default-limit constants, seed flag, and all connection-wiring
wrapper functions that delegate to ``application/monitoring/store.py`` and
``application/monitoring/reporting.py``.
    ...
    merged = {
        ...
        "max_execution_seconds": int(
            updates.get(
                ...
                current.get("max_execution_seconds", DEFAULT_MAX_EXECUTION_SECONDS),
            )
        ),
...
def get_recent_blocked_runs(
    ...
    return monitoring_store.get_recent_blocked_runs(DB_PATH, hours=hours, limit=limit)
