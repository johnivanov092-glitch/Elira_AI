# -*- coding: utf-8 -*-
"""Application-layer runtime for the legacy library service API.

Provides the ``list_library_files`` / ``set_library_active`` /
``delete_library_file`` / ``build_library_context`` API used by
``api/routes/library.py``, ``api/routes/debug.py``, and
``application/tool_registry/builtins.py``.

This is a thin façade over the same SQLite database managed by
``application/library_sqlite/runtime.py``; it preserves the original
filename-keyed API surface while delegating to shared DB helpers.

No HTTP, no FastAPI imports.
"""
...
def list_library_files() -> dict[str, Any]:
    ...
    try:
        rows = conn.execute(
            "SELECT id, name, size, use_in_context, stored_path, type, source, created_at "
...
def set_library_active(filename: str, active: bool) -> dict[str, Any]:
    ...
    try:
        row = conn.execute(
            "SELECT id, name FROM files WHERE name = ? ORDER BY id DESC LIMIT 1",
            (filename,),
        ).fetchone()
        if row:
            conn.execute(
...
def delete_library_file(filename: str) -> dict[str, Any]:
    ...
    try:
        row = conn.execute(
            "SELECT id, stored_path FROM files WHERE name = ? ORDER BY id DESC LIMIT 1",
            (filename,),
        ).fetchone()
        if not row:
            return {"ok": False, "error": f"Файл не найден: {filename}"}
...
def build_library_context(
    ...
    conn = _conn()
    rows = conn.execute(
        "SELECT name, preview, stored_path FROM files "
        "WHERE use_in_context = 1 ORDER BY created_at DESC, id DESC LIMIT ?",
        (max_files,),
    ).fetchall()
    conn.close()
