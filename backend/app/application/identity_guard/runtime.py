# -*- coding: utf-8 -*-
"""Application-layer runtime for identity-guard post-processing.

Detects identity questions (who are you?) and prevents the LLM from
claiming to be an open-source model instead of the configured persona.
...
def is_identity_question(user_input: str) -> bool:
    """Return True if *user_input* is asking about the assistant's identity."""
    return bool(IDENTITY_QUESTION_RE.search(user_input or ""))


def guard_identity_response(
    user_input: str, answer_text: str, persona_name: str = "Elira"
) -> dict[str, Any]:
    """Post-process *answer_text* to prevent persona identity drift.

    Returns a dict with:
      text: str              — processed answer text
      changed: bool          — whether the text was modified
      reason: str | None     — "identity_question_locked" | "identity_rewritten" |
                               "identity_fragment_replaced" | None
      identity_question: bool — whether the user was asking about identity
    """
