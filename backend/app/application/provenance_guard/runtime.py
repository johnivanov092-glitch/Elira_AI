# -*- coding: utf-8 -*-
"""Application-layer runtime for provenance-guard post-processing.

Strips internal RAG/memory markers from LLM responses and rewrites
natural provenance questions into user-facing language.  Pure Python —
no HTTP, no DB.
"""
...
def is_provenance_question(user_input: str) -> bool:
    """Return True if *user_input* is asking about information sources."""
...
def _strip_raw_markers(text: str) -> str:
    """Remove internal [fact]/[memory]/[source] tags and technical headers."""
...
def _rewrite_natural_provenance(text: str) -> str:
    """Rewrite memory-sourcing phrases into natural user-facing language."""
    rewritten = text
...
def _strip_technical_source_phrases(text: str) -> str:
    """Strip source-lead patterns, bare URLs, and unwanted source sentences."""
    cleaned = text
...
def _rewrite_direct_personal_facts(user_input: str, text: str) -> str:
    """Rewrite 'меня зовут X' answers to a clean 'Тебя зовут X.' form."""
...
def guard_provenance_response(user_input: str, answer_text: str) -> dict[str, Any]:
    """Post-process *answer_text* to hide internal RAG/memory markers.

    Returns a dict with:
      text: str              — processed answer text
      changed: bool          — whether the text was modified
      reason: str | None     — "natural_provenance_reply" | "internal_source_hidden" | None
      provenance_question: bool — whether the user was asking about sources
    """
