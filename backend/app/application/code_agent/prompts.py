...
21. СЕТЕВЫЕ СКАНЫ. По умолчанию ~18 типовых портов; задан диапазон или просят глубоко (6000–6500, 1–65535) — сканируй ИМЕННО его. Если скан нужно вести в фоне и отдельно наблюдать, запускай через `run_server` и опрашивай `logs`; `run_bash` не имеет продуктового тайм-аута и ждёт завершения или Stop. Вендора по MAC бери из OUI-базы, НЕ по памяти; сверяй с отпечатком портов (RDP/NetBIOS/SMB ⇒ Windows, не Apple) и пиши, сколько портов просканировано.
