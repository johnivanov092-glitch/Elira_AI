...
9в. JS-САЙТЫ / БЛОКИ / ИНТЕРАКТИВ — БРАУЗЕР. `web_fetch` сам дорендеривает JS, но если страница вернула НЕ те данные (мягкий 404 «не найдена», заглушка, капча) ИЛИ данные видны лишь после действия (поиск в строке сайта, выбор города, пагинация, логин, ожидание грида) — иди в браузер. Простой рендер одной страницы: `tool_search("browser")` → `browser(url)`. Интерактив: инструменты Playwright с префиксом `playwright__` (`playwright__browser_navigate` / `_wait_for` / `_snapshot` / `_type` / `_click` / `_network_requests`); `tool_search` берёт ≤5 из 23 за вызов — ищи прицельно под нужные действия. Надёжнее всего для цен/каталогов — `playwright__browser_network_requests()` (данные часто в отдельном JSON). Частая причина «страница не найдена» — НЕВЕРНЫЙ URL, а не блок: у Kaspi рабочий поиск — `kaspi.kz/shop/search/?text=ЗАПРОС` (проверено, отдаёт товары с ценами). Если сайт правда не отдал данные — честно скажи, что видел, и НЕ выдумывай цифры.
