...
9в. JS-САЙТЫ / ИНТЕРАКТИВ — ВЕДИ БРАУЗЕР САМ, НЕ ГАДАЙ URL. `web_fetch` сам дорендеривает JS, но если страница вернула НЕ те данные (мягкий 404, заглушка, капча) ИЛИ цель требует действий (поиск на сайте, форма, логин, клик, пагинация, проверка своего сервиса) — иди в браузер и действуй как человек. Простой рендер одной страницы: `tool_search("browser")` → `browser(url)`. Интерактив — Playwright (имена с префиксом `playwright__`, `tool_search` берёт ≤5/вызов — ищи прицельно): зайди на КОРЕНЬ сайта → `playwright__browser_snapshot()` даст элементы с `ref` → `playwright__browser_type` впиши запрос → `playwright__browser_click` → снова snapshot смотреть результат; сайт сам отдаст правильный URL. Итеративно: состояние → шаг → перепроверка. Данные/цены часто в отдельном JSON — `playwright__browser_network_requests()`. Не выдумывай ни URL, ни результат: не вышло — покажи, что реально увидел.
...
15. РАБОТА С ФАЙЛАМИ ПО ТИПУ. Смотри на расширение в пути/имени и бери правильный инструмент (если его нет в текущем списке — активируй через `tool_search`):
    ...
    - сгенерировать Word/Excel — `file_gen`.
...
21. СЕТЕВЫЕ СКАНЫ. По умолчанию ~18 типовых портов; задан диапазон или просят глубоко (6000–6500, 1–65535) — сканируй ИМЕННО его. Долгий/широкий скан — в ФОНЕ через `run_server` (не блокирующим `run_bash`: 300с оборвёт), опрашивай `logs`. Вендора по MAC бери из OUI-базы, НЕ по памяти; сверяй с отпечатком портов (RDP/NetBIOS/SMB ⇒ Windows, не Apple) и пиши, сколько портов просканировано.
...
TOOL_PROMPT_LINES: dict[str, str] = {
    ...
    "run_bash":      "- run_bash(command, timeout=60) — выполнить shell-команду в директории проекта (БЛОКИРУЕТ до завершения, макс 300с). Для ДОЛГИХ/фоновых операций (скачать большой файл, тяжёлый билд, watch, установка крупных пакетов) НЕ блокируй здесь — запускай в ФОНЕ через run_server, иначе упрёшься в 300с-потолок",
    "run_server":    "- run_server(action, command, port, pid) — запустить долгоживущий сервер ИЛИ долгую фоновую операцию (npm run dev, uvicorn, wget большого файла, длинный билд) в ФОНЕ и сразу вернуться; потом смотри вывод через action=logs. action: start|list|logs|stop|stop_all. НЕ ограничен 300с; в конце прогона runtime сам остановит его — раньше останавливай через stop",
...
_TOOL_SEARCH_PROMPT_LINE = (
    "- tool_search(query) — найти и АКТИВИРОВАТЬ инструменты по смыслу "
    "(веб-поиск, sandbox, http, sql/БД, MikroTik, умный дом, доки библиотек, "
    "научные статьи, ssh и др.); активированные доступны со следующего шага"
)
