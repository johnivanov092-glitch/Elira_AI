# -*- coding: utf-8 -*-
"""Application-layer runtime for temporal-intent detection.

Classifies whether a user query requires fresh web data (hard/soft/none mode)
based on relative-time terms, current-world terms, explicit year references,
and historical-pattern matching.  Pure Python — no HTTP, no DB.
"""
...
def detect_temporal_intent(query: str, now: datetime | None = None) -> dict[str, Any]:
    ...
    Returns a dict with:
      mode: "hard" | "soft" | "stable_historical" | "none"
      requires_web: bool
      freshness_sensitive: bool
      analytic: bool
      years: list[int]
      signals: list[str]
      reasoning_depth: "none" | "standard" | "deep"
      current_year: int
      stable_historical: bool
    """
    text = (query or "").strip()
