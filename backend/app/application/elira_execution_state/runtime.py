...
def persist_state(goal: str, checkpoints: list, queue_items: list, rollback: dict) -> int:
    ensure_db()
    conn = connect_sqlite(DB_PATH, row_factory=None, journal_mode=None)
    try:
        cur = conn.execute(
...
def prepare_execution_state(goal: str, queue_items: list, staged_paths: List[str]) -> dict:
    ...
    state["state_id"] = persist_state(goal, checkpoints, queue_items, rollback)
    return state
