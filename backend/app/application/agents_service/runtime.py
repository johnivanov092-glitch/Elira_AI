"""Application-layer runtime for the Chat Agent composition root.

Assembles the ChatAgentDeps struct by wiring all application-layer
sub-services, then exposes run_agent() and run_agent_stream() as the
canonical entry points for chat execution.
"""
...
from app.application.agent_sandbox.runtime import preflight_or_raise, resolve_effective_agent_id
from app.application.chat_service.runtime import run_chat, run_chat_stream
...
from app.application.smart_memory.search import get_relevant_context
from app.application.tool_service.runtime import run_tool

try:
    from app.application.rag_memory_service.runtime import get_rag_context
