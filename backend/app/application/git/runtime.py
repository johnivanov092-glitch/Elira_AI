# -*- coding: utf-8 -*-
"""Application-layer runtime for Git integration.

Owns all subprocess-based Git operations.  Pure Python — no HTTP, no DB,
no FastAPI imports.  All Russian user-facing strings kept as valid UTF-8.
"""
...
def git_status(repo_path=None):
    ...
    files = [
        {"status": l[:2].strip(), "file": l[3:].strip()}
    ...
    return {
        ...
        "clean": not files,
        "summary": (
            f"Ветка: {branch} | {len(files)} изменений"
            if files
            else f"Ветка: {branch} | Чисто"
        ),
    }
...
def git_diff(repo_path=None, file_path=None):
    ...
    return {
        "ok": True,
        "stat": _run(cmd_stat, cwd=cwd)["stdout"],
        "diff": (_run(cmd_diff, cwd=cwd)["stdout"] or "Нет изменений")[:8000],
        "repo": cwd,
    }
