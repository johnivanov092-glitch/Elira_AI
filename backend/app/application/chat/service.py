...
# ─── original service.py helpers (kept for backward compatibility) ────────────
...
def build_disabled_skills(
    *,
    ...
    return {name for name, enabled in skill_flags.items() if not enabled}
...
def prepare_chat_plan(
    *,
    ...
    web_plan = dict(raw_web_plan) if isinstance(raw_web_plan, dict) and raw_web_plan else {
        "is_multi_intent": False, "subqueries": []
    }

    selected_tools = [
        t for t in list(plan.get("tools", []) or [])
    ...
    if route == "research" and temporal.get("mode") == "hard" and temporal.get("freshness_sensitive"):
        selected_tools = [t for t in selected_tools if t != "memory_search"]
