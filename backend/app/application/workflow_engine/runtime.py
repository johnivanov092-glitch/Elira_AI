# -*- coding: utf-8 -*-
"""Application-layer runtime for the Workflow Engine service (Agent OS Phase 4).

Owns ``_resolved_db_path()`` and all delegation wrapper functions over
``application/workflows/``.  Also re-exports the multi-agent workflow
constants and functions from ``application/workflows/multi_agent``.
...
from app.application.workflows.multi_agent import (
...
def cancel_workflow_run(run_id: str) -> dict[str, Any]:
    return _app_cancel_workflow_run(run_id, db_path=_resolved_db_path())
