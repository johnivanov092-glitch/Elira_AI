# -*- coding: utf-8 -*-
"""Application-layer runtime for Elira settings.

Owns ``get_settings`` / ``save_settings`` / ``get_route_model_map`` —
all persisted in the ``settings`` table of ``elira_state.db``.
...
DEFAULT_ROUTE_MAP: dict[str, list[str]] = {
...
def _ensure_route_map_column() -> None:
    ...
    try:
        ...
        if "route_model_map" not in columns:
            conn.execute(
                "ALTER TABLE settings ADD COLUMN route_model_map TEXT DEFAULT '{}'"
            )
            conn.execute(
...
def save_settings(
    ...
    try:
        conn.execute(
            """
            UPDATE settings
            SET ollama_context = ?, default_model = ?, agent_profile = ?,
                route_model_map = ?
            WHERE id = 1
            """,
