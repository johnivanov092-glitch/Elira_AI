# -*- coding: utf-8 -*-
"""Application-layer runtime for the agent sandbox.

Enforces preflight policies (context-window limits, tool allow-lists, hourly
rate limits) before an agent run is permitted.  All DB calls are delegated to
``services/agent_monitor`` which owns the monitoring DB.  No HTTP layer.
"""
...
# ── profile → builtin-agent-id mapping ───────────────────────────────────────
# Russian prefixes are used for fuzzy profile-name matching.
...
def _normalize_tool_names(
    selected_tools: list[str] | tuple[str, ...] | None,
) -> list[str]:
    seen: set[str] = set()
    tools: list[str] = []
...
def evaluate_preflight(
    *,
    ...
    allowed_tools = {
        str(item or "").strip()
        for item in limit.get("allowed_tools", [])
    ...
    if max_runs_per_hour > 0:
        ...
        if recent_runs >= max_runs_per_hour:
            raise _make_error(
                ...
                message=(
                    f"Agent sandbox blocked run: hourly run limit reached "
                    f"({recent_runs}/{max_runs_per_hour})."
                ),
                details={
