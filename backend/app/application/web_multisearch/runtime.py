# -*- coding: utf-8 -*-
"""Application-layer runtime for multi-engine web search.

Thin orchestration wrappers over ``core/web.py`` — all heavy HTTP work
is delegated via lazy imports.  Returns plain dicts; no HTTP, no DB at
module level.
"""
...
def multi_search(
    ...
    Returns:
      ok, query, results, count, engines, formatted
    """
    try:
...
def deep_search(
    ...
    """Search + parallel page content fetch.

    Returns:
      ok, query, content, content_length
    """
    try:
...
def news_search(query: str, max_results: int = 5) -> dict[str, Any]:
    """Fresh news search via DDG News.

    Returns:
      ok, query, items, count
    """
    try:
...
class WebMultiSearchService:
    """Object-oriented façade over the module-level search functions."""
    ...
    def search(
        ...
        return multi_search(
            query,
            engines=engines or ("tavily", "duckduckgo", "wikipedia"),
