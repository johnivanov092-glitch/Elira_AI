# -*- coding: utf-8 -*-
"""Application-layer runtime for the Run History service.

Owns DB_PATH, LEGACY_JSON_PATHS, _MAX_RUNS, connection wiring, and the
backward-compatible ``RunHistoryService`` class that subclasses
``application/run_history/runtime.RunHistoryService``.
...
LEGACY_JSON_PATHS = [
    ...
    Path(__file__).resolve().parents[3] / "data" / "run_history.json",
]
_MAX_RUNS = 200
...
class RunHistoryService(RunHistoryRuntime):
    """Backward-compatible RunHistoryService wired to the project DB_PATH."""
