# -*- coding: utf-8 -*-
"""Application-layer runtime for backend status aggregation.

Collects Python runtime, storage, persona, and web-engine status into a
single dict.  Imports from ``core/`` and other services are at module
level (they are all pure-Python constants or lightweight helpers with no
...
def init_runtime_state() -> dict[str, Any]:
    """Initialise the DB and return the current runtime status dict."""
    init_db()
...
def get_runtime_status() -> dict[str, Any]:
    ...
    Covers: Python executable/PID/CWD, storage paths, active chat count,
    persona version, web engine availability, and any degraded-mode warnings.
    """
    init_db()
