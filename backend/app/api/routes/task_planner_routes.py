...
def api_list(status: str | None = None, category: str | None = None, limit: int = 100):
    from app.application.planning.task_planner_service import list_tasks
    return list_tasks(status=status, category=category, limit=limit)


@router.post("/create")
def api_create(req: CreateTaskRequest):
    from app.application.planning.task_planner_service import create_task
    return create_task(
        title=req.title,
...
@router.get("/get/{tid}")
def api_get(tid: str):
    from app.application.planning.task_planner_service import get_task
    return get_task(tid)


@router.put("/update/{tid}")
def api_update(tid: str, req: UpdateTaskRequest):
    from app.application.planning.task_planner_service import update_task
...
def api_delete(tid: str):
    from app.application.planning.task_planner_service import delete_task
    return delete_task(tid)


@router.get("/stats")
def api_stats():
    from app.application.planning.task_planner_service import task_stats
    return task_stats()
