...
class CodeAgentRequest(BaseModel):
    ...
    # Retained for wire compatibility and injected/offline runtimes. Live runs
    # always use the exact llama.cpp /props n_ctx.
...
class SummarizeHistoryRequest(BaseModel):
    ...
    # Retained for wire compatibility and injected/offline runtimes. Live
    # summarization resolves the exact llama.cpp /props n_ctx.
    num_ctx: Optional[int] = Field(default=None, ge=1024)
...
def read_context_profile(
    model: str = DEFAULT_MODEL, num_ctx: Optional[int] = None,
) -> dict[str, Any]:
    """Initial usage for the existing composer meter.

    ``num_ctx`` is accepted only for backward wire compatibility and is ignored
    on this live route. A fresh /props read keeps the meter aligned with the
    model currently loaded at the server URL.
    """
    ...
    try:
        profile = resolve_context_window(
            None, model=model, live=True, fresh=True,
        )
...
def summarize(payload: SummarizeHistoryRequest) -> SummarizeHistoryResponse:
    ...
    result = summarize_history(
        messages=messages, model=payload.model, num_ctx=payload.num_ctx,
    )
