"""Tests for application/telegram (store DB CRUD + runtime helpers) and
application/browser_agent (stub class)."""
...
from app.application.browser_agent.runtime import BrowserAgent  # noqa: E402


# ─────────────────────────────────────────────────────────────────────────────
# telegram/store.py — DB CRUD with patched DB_PATH
# ─────────────────────────────────────────────────────────────────────────────
...
class TelegramStoreCRUDTest(unittest.TestCase):
    ...
    # ── register_user / is_user_allowed ──────────────────────────────────────

    def test_register_user_stores_user(self) -> None:
        # register_user returns None; verify via list_telegram_users
    ...
    def test_is_user_allowed_for_all(self) -> None:
        # Default allowed_users = "all" → all users allowed
    ...
    def test_is_user_allowed_for_none(self) -> None:
        # allowed_users = "none" → nobody allowed
...
# ─────────────────────────────────────────────────────────────────────────────
# telegram/runtime.py — get_telegram_config, telegram_bot_status
# ─────────────────────────────────────────────────────────────────────────────
...
class TelegramRuntimeConfigTest(unittest.TestCase):
    ...
    def test_get_config_has_required_keys(self) -> None:
        result = tg_rt.get_telegram_config()
        for key in ("has_token", "model", "profile", "running",
                    "use_memory", "use_web_search", "max_message_length"):
...
# ─────────────────────────────────────────────────────────────────────────────
# browser_agent — BrowserAgent stub
# ─────────────────────────────────────────────────────────────────────────────
...
class BrowserAgentStubTest(unittest.TestCase):
    ...
    def test_all_methods_consistently_not_ok(self) -> None:
        # search requires query arg; run and screenshot have no required args
        results = [
