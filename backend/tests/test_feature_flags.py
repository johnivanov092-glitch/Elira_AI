    ...
    def test_all_flags_off_with_no_file_no_env(self) -> None:
        with tempfile.TemporaryDirectory() as data:
            try:
                ...
                self.assertEqual(
                    ff.get_flags(),
                    {"remote_mcp": False, "action_envelopes": False, "proactive": False, "catalog_assist": False, "web_corpus": False, "itops": False},
                )
            finally:
                _clear_env()
    ...
    def test_set_flag_persists_and_returns_state(self) -> None:
        with tempfile.TemporaryDirectory() as data:
            try:
                ...
                self.assertEqual(
                    state, {"remote_mcp": False, "action_envelopes": True, "proactive": False, "catalog_assist": False, "web_corpus": False, "itops": False}
                )
        ...
        with tempfile.TemporaryDirectory() as data:
            try:
                ...
                self.assertEqual(
                    ...
                    {"remote_mcp", "action_envelopes", "proactive", "catalog_assist", "web_corpus", "itops"},
                )
            finally:
                _clear_env()
