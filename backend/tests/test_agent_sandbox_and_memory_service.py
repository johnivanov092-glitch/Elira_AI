"""Tests for application/agent_sandbox (pure helpers + mocked preflight) and
application/memory_service (facade over smart_memory with patched DB_PATH)."""
...
from app.application.agent_sandbox.runtime import (  # noqa: E402
...
import app.application.memory_service.runtime as ms_rt  # noqa: E402
...
class EvaluatePreflightTest(unittest.TestCase):
    ...
    def _mock_limit(self, limit_dict: dict):
        return patch(
            "app.application.agent_sandbox.runtime.ensure_agent_limit",
    ...
    def test_preflight_raises_on_rate_limit(self) -> None:
        ...
        with self._mock_limit(limit), patch(
            "app.application.agent_sandbox.runtime.count_agent_runs_last_hour",
            return_value=3,
        ):
            with self.assertRaises(SandboxPolicyError) as ctx:
