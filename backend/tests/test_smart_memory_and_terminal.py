...
class GetCwdTest(unittest.TestCase):
    def test_get_cwd_returns_dict(self) -> None:
        result = term_rt.get_cwd()
        self.assertTrue(result["ok"])
        self.assertIsInstance(result["cwd"], str)

    def test_get_cwd_is_not_empty(self) -> None:
        self.assertGreater(len(term_rt.get_cwd()["cwd"]), 0)

    def test_get_cwd_matches_module_state(self) -> None:
        self.assertEqual(term_rt.get_cwd()["cwd"], term_rt._cwd)
...
class ExecCommandTest(unittest.TestCase):
    ...
    def test_blocked_rm_rf(self) -> None:
        ...
        self.assertFalse(result["ok"])
        self.assertIn("rm -rf /", result["error"].lower())
    ...
    def test_exec_timeout_returns_error(self) -> None:
        ...
        self.assertFalse(result["ok"])
        self.assertIn(str(term_rt.TIMEOUT), result["error"])
