...
- **SearXNG pageno** — `web_search(page=2..5)`: глубже по выдаче того же
  ...
  недоступном SearXNG ошибка, не тихая первая страница. Параметр `page` в схеме
  только при флаге.
- **Tier-теги (детерминированная эвристика-guard):** `tiers.py classify_tier` —
  курируемые списки + структурные правила (docs./developer.-хосты, .gov-TLD);
  дефолт `unknown` (не угадываем). Записывается в корпус при ingest, показывается
  в выдаче web_search (`[official]`/`[primary]`/`[secondary]`/`[ugc]`) и в
  реестре цитат. Инвариант №9: одноисточниковый official аннотируется «может
  быть достаточен один», но счётчик одноисточниковых остаётся честным.
  Tier ≠ вердикт истинности и не заменяет провенанс.
- **Politeness-бюджет per-domain:** `politeness.py` — два подряд fetch'а к
  ...
  разные домены не троттлятся; ожидание ограничено сверху.
