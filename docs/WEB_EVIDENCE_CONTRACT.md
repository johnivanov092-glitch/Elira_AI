...
## W6 — ширина (✅ реализовано)

Три мелких элемента ширины, все за флагом web_corpus (бит-в-бит при off):
- **SearXNG pageno** — `web_search(page=2..5)`: глубже по выдаче того же
  запроса. SearXNG-only честно: fallback-движки пагинацию не умеют → при
  недоступном SearXNG ошибка (**ok=False** — executor не зеленит), не тихая
  первая страница. ⚠ Флаг-гейт ЖИВЁТ В САМОМ ТУЛЕ, не только в схеме — action
  envelope передаёт любые аргументы, схемного гейта недостаточно для
  бит-в-бит-off (ревью W6). Валидация строгая: 1..5, page>1 только с одиночным
  query. Тесты гейтов — через executor-путь, не только прямые вызовы.
- **Tier-теги (детерминированная эвристика-guard):** `tiers.py classify_tier` —
  ТОЛЬКО курируемые registrable-домены + gov/mil-TLD; дефолт `unknown` (не
  угадываем). ⚠ Структурные host-правила (docs.*/wiki.*) ЗАПРЕЩЕНЫ — имя
  сабдомена контролирует владелец, docs.evil.example получал official и
  эскалацию доверия в реестре (ревью W6). Записывается в корпус при ingest,
  показывается в выдаче web_search (`[official]`/`[primary]`/`[secondary]`/
  `[ugc]`) и в реестре цитат. Инвариант №9: одноисточниковый official
  аннотируется «может быть достаточен один», но счётчик одноисточниковых
  остаётся честным. Tier ≠ вердикт истинности и не заменяет провенанс.
- **Politeness-бюджет per-domain:** `politeness.py` — два подряд fetch'а к
  одному registrable-домену разнесены ≥1с (corpus._fetch_raw + sitemap._get);
  разные домены не троттлятся; ожидание ограничено сверху. **Deadline-aware**
  (ревью W6): в sitemap ожидание, вылезающее за 20с-бюджет, НЕ спит — abort с
  честной time-budget ошибкой; requests получает ОСТАТОК бюджета (cap 10с).
...
- **Корроборация (детерминированно):** на claim считается число НЕЗАВИСИМЫХ
  источников = различных registrable-доменов (eTLD+1) среди подтверждённых
  цитат. 0 → нет источника; 1 → «одноисточниково, не перекрёстно» (флаг);
  ≥2 → «перекрёстно: N независимых домена». eTLD+1 — bounded-эвристика с набором
  multi-part TLD (co.uk и т.п.); приближение независимости (инвариант №9).
  Один и тот же домен ≠ перекрёстное подтверждение.
- **Свежесть:** дата источника (`published`/`modified` из meta-тегов +
  `Last-Modified` заголовок) захватывается при ingest в поле `dates`. Render
  показывает дату; источник старше `STALE_YEARS`=3 → «⚠ возможно устарел».
  Дата детерминирована; «устарел» — порог-эвристика (caution, не вердикт).
  Неизвестная дата → «дата неизвестна» (не наказываем отсутствие метаданных).
- **Противоречия (advisory):** модель помечает claim `conflicted` через
  web_claim_add (инвариант №8, NLI-класс никогда не runtime-вердикт) → отдельная
  секция «⚠ Противоречия (отмечено моделью — advisory)».

Модуль `web_evidence/freshness.py` — чистые функции над сохранёнными
метаданными, без сети и нового провайдера. Тесты: eTLD+1 (вкл. multi-part и IP),
extract_dates (meta+header), stale-порог, уровни корроборации, render
single/multi/same-domain/stale/conflict.
