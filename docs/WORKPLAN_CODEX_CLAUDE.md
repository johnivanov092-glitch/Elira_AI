...
| `3` | Continue routing SQLite consumers through `app.infrastructure.db.connection` | `codex/refactor-arch-foundation` | `response_cache.py`, `run_history_service.py`, `elira_memory_sqlite.py`, and `elira_settings_sqlite.py` are migrated; next prefer another small service-level consumer before touching route-level DB files |
