...
| `3` | Continue routing SQLite consumers through `app.infrastructure.db.connection` | `codex/refactor-arch-foundation` | `response_cache.py`, `run_history_service.py`, `elira_memory_sqlite.py`, `elira_settings_sqlite.py`, `persona_service.py`, `rag_memory_service.py`, `library_service.py`, `task_planner_service.py`, `autopipeline_service.py`, `smart_memory.py`, `telegram_service.py`, and the SQLite paths in `skills_service.py` are migrated; next only continue if another small service-level consumer remains before touching route-level DB files |
