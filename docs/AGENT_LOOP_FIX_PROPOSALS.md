...
**Дата:** 2026-06-10
**Статус:** отложено в post-server backlog. До переноса инференса на отдельный
AI-server этот документ не является активным планом работ. После успешного
подключения Elira к серверному OpenAI-compatible endpoint вернуться к пунктам F1-F8,
сначала заново сверить код и только потом принимать шаги в работу.
