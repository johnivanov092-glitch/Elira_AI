...
|---|---|---|---|
| G1 | ~~Runtime не читает каталог~~ — ✅ закрыто R1 как ASSIST (дрейф-лог + unsupported-label + прозовые хинты; НЕ классификатор, под флагом) | — | классификация остаётся в коде намеренно |
...
**R1 — Каталог как runtime-ASSIST — ✅ DONE (флаг `catalog_assist`, default OFF)**
- НЕ классификатор (осознанно): runtime читает `verifier_catalog.yaml` для трёх вещей —
  (а) **дрейф-лог**: intent из кода, которого каталог не знает → warning (анти-дрифт
  канарейка в тестах пинует полноту `meta.intents`; сразу поймала отсутствовавший
  `command_output`); (б) **честный label «нет верификатора»** в readiness для критериев
  без верификатора (generic/report — floor даже при недоступном каталоге); (в) **прозовые
  негативные правила** каталога в closure-хинтах (bare-токены отфильтрованы).
- Fail-open на каждом слое: битый yaml/нет PyYAML → no-op (одно предупреждение на mtime,
  не на вызов); при выключенном флаге поведение бит-в-бит (ключ `unsupported` отсутствует,
  nudge-текст идентичен — пиновано тестами). UI-тумблер в «Экспериментальном».
- Ревью (11 находок, verify-фаза в лимите → ручной триаж, все реальные, все пофикшены):
  главное — PUT-Literal без нового флага (точный повтор D3-бага: тумблер бы 422-ил),
  полный try вокруг парсинга (malformed-but-parseable yaml ронял ран), notes-фильтр
  (голые intent-токены шли бы в промпт), кэш неудачной загрузки, phantom-intent от
  `cleanup.confirmed`, env=""≠off в тесте.
