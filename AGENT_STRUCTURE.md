...
После успешного пользовательского ответа основной code-agent передаёт диалог в
существующий persona evolution runtime. `session_id` проходит через journal и
Resume, поэтому несколько запусков одного чата не считаются независимыми
сессиями. Внутренние subagent-запуски и вызовы без пользовательского
`session_id` личность не обучают. Кандидаты остаются в карантине до выполнения
порогов независимых диалогов, сессий, уверенности и непротиворечивости; после
promotion их черты приоритетно, но в фиксированном лимите входят в следующий
persona prompt.
